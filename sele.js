  ...
  async exec(task, account) {
    ...
    try {
      ...
      if (await this.isExistEle(se[0], true, 3000)) {
        ...
        inputEle.sendKeys(account.id);
        // パスワード入力
        inputEle = await this.getEle(se[1], 500);
        await inputEle.clear();
        inputEle.sendKeys(account.password);
        ...
        if (await this.isExistEle(se[2], true, 3000)) {
          // 照会　上の条件で
          let el = await this.getEle(se[2], 2000);
          await this.clickEle(el, 2000);
          se = ["div.train:not(.saleEnd)", "div.stationInfo>div", "ancestor::div[@class='train']", "div.trainInfo a.submitButton"];
          if (await this.isExistEle(se[0], true, 3000)) {
            let els = await this.getEles(se[0], 2000);
            let isBreak = false;
            for (let i in els) {
              if (await this.isExistElesFromEle(els[i], se[1], true, 3000)) {
                let el0 = await this.getElesFromEle(els[i], se[1], 2000);
                for (let j in el0) {
                  let text = await el0[j].getProperty("innerText");
                  logger.debug(text);
                  if (text.indexOf("高幡不動") > -1 && text.indexOf(task.time) > -1) {
                    let el2 = await this.getElesXFromEle(el0[j], se[2], 2000);
            ...
            if (await this.isExistEle(se[1], true, 3000)) {
              ...
              if (await this.isExistEle(se[1], true, 3000)) {
                let els = await this.getEles(se[1], 1000);
                for (let el1 of els) {
                  let seatNum = await el1.getText();
                  if (seatNum.trim() == task.s_num) {
                    let el2 = await this.getElesXFromEle(el1, se[3], 2000);
                    await this.clickEle(el2[0], 1000);
                    break;
                  }
                }
                if (await this.isExistEle(se[2], true, 3000)) {
                  let el = await this.getEle(se[2], 1000);
                  ...
                  if (await this.isExistEle(se[0], true, 3000)) {
                    ...
                    if (await this.isExistEle(se[1], true, 3000)) {
                      ...
                      if (await this.isExistEle(se[2], true, 3000)) {
                        let el = await this.getEle(se[2], 1000);
                        await this.clickEle(el, 1000); // 遷移
                        if (await this.isExistEle(se[3], true, 3000)) {
