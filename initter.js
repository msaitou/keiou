  ...
  log.configure({
    appenders: {
      ...
      out: {
        ...
        layout: {
          type: "pattern",
          pattern: "[%d{yy-MM-dd hh:mm:ss} %[%.4p%]] %m ->%f{2} %l",
        },
      },
      app: {
        type: "dateFile",
...
exports.initBrowserDriver = async function (isMob = false, headless = true) {
  ...
  if (conf.proxy && conf.proxy.host && conf.proxy.port)
    chromeOptions.addArguments(
      `--proxy-server=http://${conf.proxy.host}:${conf.proxy.port}`
    );
...
const getDownloadUrl = async () => {
  ...
  if (fs.existsSync(localDriverFullPath))
    localDriverVer = await getNowChromeVer(
      localOS === "win64"
        ? localDriverFullPath.split("\\").join("\\\\")
        : localDriverFullPath,
      true
    );
  console.info(
    `localOS: ${localOS} local chromeDriverVersion:${localDriverVer} now chromeVersion:${browseVer}`
  );
      ...
      req({ url, timeout: 180000 }, async (err, res, body) => {
        ...
        if (dObj && dObj.milestones[browseVer]) {
          ...
          if (dObj.milestones[browseVer].downloads.chromedriver) {
            let targetLine = dObj.milestones[
              browseVer
            ].downloads.chromedriver.filter((l) => l.platform === localOS)[0];
            if (targetLine) {
              ...
              if (browseVer != localDriverVer) {
                ...
                // 解凍
                let zip = new AdmZip(
                  CHROME.DRIVER.DIR + CHROME.DRIVER.DOWNLOAD_NAME
                );
                zip.extractAllTo(
                  /*target path*/ CHROME.DRIVER.DIR,
                  /*overwrite*/ true
                );
                for (const zipEntry of zip.getEntries()) {
                  ...
                  if (
                    `${CHROME.DRIVER.NAME}${CHROME.DRIVER.EXTENSION[localOS]}` ==
                    fName
                  ) {
                    // ドライバーを所定の位置に移動
                    fs.renameSync(
                      CHROME.DRIVER.DIR + zipEntry.entryName,
                      CHROME.DRIVER.DIR + fName
                    ); // デフォルト上書き
                    if (localOS === "linux64") {
                      try {
                        ...
                        console.log(
                          "The permissions for chromedriver have been changed!"
                        );
                        throw e;
                      }
                    }
...
const downloadDriver = async (url) => {
  return new Promise(async (res, rej) => {
    try {
      req(url)
        .pipe(
          fs.createWriteStream(CHROME.DRIVER.DIR + CHROME.DRIVER.DOWNLOAD_NAME)
        )
        .on("close", function () {
...
const getNowChromeVer = async (fullPath, isDriver) => {
  return new Promise(async (res, rej) => {
    ...
    await exec(cmd, (err, stdout, stderr) => {
      ...
      if (isDriver)
        browseVer = stdout.trim().replace("ChromeDriver ", "").split(".")[0];
      else if (os === "linux64" && !isDriver)
        browseVer = stdout.trim().replace("Google Chrome ", "").split(".")[0];
